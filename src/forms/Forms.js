...
export default function Forms() {
  ...
  const duplicateForm = async (form) => {
    try {
      await formService.duplicateForm(userId, form._id);
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${filter === 'draft'
                ? 'bg-primary-100 text-primary-700'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
            >
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 mt-4 sm:mt-0 sm:ml-4">
                    {form.participantCount > 0 && (
                      <Link
                        to={`/dashboard/forms/${form._id}/responses`}
