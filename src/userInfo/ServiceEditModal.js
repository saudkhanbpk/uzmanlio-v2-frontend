...
export const ServiceEditModal = ({ onClose, service }) => {
  const { updateService, loading } = useExpertData();
  ...
  useEffect(() => {
    if (service) {
      setFormData({
        ...
        deliverables: service.deliverables ? service.deliverables.join('\n') : ''
      });
    }
  }, [service]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateService(userId, service.id, serviceData);
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Teslim Edilecekler</label>
            <textarea
              value={formData.deliverables}
              onChange={(e) => setFormData({...formData, deliverables: e.target.value})}
              placeholder="Her satıra bir teslim edilecek yazın..."
              rows={3}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            />
            <p className="text-xs text-gray-500 mt-1">Her satıra bir teslim edilecek yazın</p>
          </div>
          ...
          <div className="flex space-x-3 pt-4">
            ...
            >
              {loading.services ? 'Güncelleniyor...' : 'Güncelle'}
            </button>
          </div>
        </form>
      </div>
