...
export const PackageEditModal = ({ onClose, package: packageData }) => {
  const { updatePackage, loading } = useExpertData();
  ...
  useEffect(() => {
    if (packageData) {
      setFormData({
        ...
        maxSessions: packageData.maxSessions?.toString() || '',
        validityPeriod: packageData.validityPeriod?.toString() || ''
      });
    }
  }, [packageData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updatePackage(userId, packageData.id, updatedPackageData);
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Paket Düzenle</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex space-x-3 pt-4">
            ...
            >
              {loading.packages ? 'Güncelleniyor...' : 'Güncelle'}
            </button>
          </div>
        </form>
      </div>
